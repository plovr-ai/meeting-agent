...
final class TranscriptFileWriterTests: XCTestCase {
    ...
    func testUpsertInterimSegmentDoesNotDuplicateOverlappingFinalWithSharedPhrase() throws {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("probe-transcript-\(UUID().uuidString).txt")
        let jsonURL = url.deletingPathExtension().appendingPathExtension("json")
        defer {
            try? FileManager.default.removeItem(at: url)
        ...
        try writer.upsert(TranscriptSegment(
            id: "deepgram-transcribe-stream-39.89",
            speaker: speaker,
            startTimeSeconds: 39.89,
            endTimeSeconds: 42.69,
            text: "Microsoft Teams meetings and basically does speech to speech",
            sourceProvider: "deepgram-transcribe",
            isFinal: true,
            timingSource: .precise
        ))
        try writer.upsert(TranscriptSegment(
            id: "deepgram-transcribe-stream-40.13",
            speaker: speaker,
            startTimeSeconds: 40.13,
            endTimeSeconds: 43.81,
            text: "Teams meetings and basically does speech to speech translations for",
            sourceProvider: "deepgram-transcribe",
            isFinal: false,
            timingSource: .precise
        ))
        ...
        let document = try TranscriptFileWriter.readDocument(from: jsonURL)
        XCTAssertEqual(document.segments.map(\.id), ["deepgram-transcribe-stream-39.89"])
        XCTAssertEqual(document.segments.map(\.isFinal), [true])
    }

    func testUpsertFinalSegmentReplacesOverlappingInterimWithInflectedText() throws {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("probe-transcript-\(UUID().uuidString).txt")
        let jsonURL = url.deletingPathExtension().appendingPathExtension("json")
        defer {
            try? FileManager.default.removeItem(at: url)
        ...
        let writer = try TranscriptFileWriter(url: url)
        let speaker = TranscriptSpeaker(identifier: "deepgram-speaker-2")

        try writer.upsert(TranscriptSegment(
            id: "deepgram-transcribe-stream-55.35",
            speaker: speaker,
            startTimeSeconds: 55.35,
            endTimeSeconds: 58.55,
            text: "Yes. Incredible. I listen to",
            sourceProvider: "deepgram-transcribe",
            isFinal: false,
            timingSource: .precise
        ))
        try writer.upsert(TranscriptSegment(
            id: "deepgram-transcribe-stream-55.43",
            speaker: speaker,
            startTimeSeconds: 55.43,
            endTimeSeconds: 59.11,
            text: "Yes. Incredible. I listened to Jerome.",
            sourceProvider: "deepgram-transcribe",
            isFinal: true,
            timingSource: .precise
        ))
        ...
        let document = try TranscriptFileWriter.readDocument(from: jsonURL)
        XCTAssertEqual(document.segments.map(\.id), ["deepgram-transcribe-stream-55.43"])
        XCTAssertEqual(document.segments.map(\.text), ["Yes. Incredible. I listened to Jerome."])
        XCTAssertEqual(document.segments.map(\.isFinal), [true])
    }
