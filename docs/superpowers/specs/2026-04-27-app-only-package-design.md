...
- CLI-only `ProbeOptions` and tests for CLI flag parsing are gone.
...
`Sources/CoreAudioTapProbe/ProbeMain.swift` and `Sources/CoreAudioTapProbe/Info.plist` will be deleted. `Sources/MeetingAgentCore/ProbeOptions.swift` will also be deleted because it only parses CLI flags and has no app consumers.

Tests will keep coverage for reusable core types and remove only CLI parser assertions from `RecordingOutputTests`. A small manifest regression test will verify that the package manifest does not reintroduce `CoreAudioTapProbe`.
...
- Run the focused manifest test and `RecordingOutputTests`.
- Run `swift test`.
